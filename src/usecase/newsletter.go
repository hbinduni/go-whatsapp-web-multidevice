...
func (service serviceNewsletter) Unfollow(ctx context.Context, request domainNewsletter.UnfollowRequest) (err error) {
	...
	JID, err := utils.ValidateJidWithLogin(whatsapp.GetClientFromContext(ctx), request.NewsletterID)
	if err != nil {
		return err
	}

	return whatsapp.GetClientFromContext(ctx).UnfollowNewsletter(ctx, JID)
}
